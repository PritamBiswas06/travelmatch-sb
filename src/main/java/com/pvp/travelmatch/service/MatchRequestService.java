...
public class MatchRequestService {
    ...
    public MatchRequest sendRequest(Long travelPlanId) {
        ...
        // so a REJECTED request must be updated in place rather than re-inserted)
        ...
        if (existing.isPresent()) {
            ...
            // REJECTED -> allow resending by resetting this same row to PENDING
            request.setStatus("PENDING");
            request.setCreatedAt(LocalDateTime.now());

        } else {

            request = MatchRequest.builder()
                    .sender(sender)
                    .receiver(receiver)
                    .travelPlan(plan)
                    .status("PENDING")
